"""Portable-package filesystem executor and compensated import use case."""
...
from .runtime import LiteCutPortablePackageJob, normalize_project_body
...
def resolve_package_destination(raw: str) -> Path | None:
...
async def rollback_portable_import(
    ...
    *,
    ...
    if project_id is None:
        return
    try:
        await delete_project_asset_files(project_id)
    except Exception:
...
async def import_portable_package(
    file,
    *,
    services,
    data_dir: Path,
    ...
    from .assets import stable_project_asset_directory, validate_asset_filename

    if not str(file.filename or "").lower().endswith(".zip"):
        raise HTTPException(400, "请选择 LiteCut 便携工程包（.zip）")
    package_dir = data_dir / "lite_cut_packages"
    package_dir.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        with zipfile.ZipFile(temp_path) as archive:
            ...
            project_id = await services.projects.create(name=name, body=normalize_project_body(raw_project["body"]))
            ...
            for item in raw_project.get("files") or []:
                ...
                new_asset = await services.assets.create(
                    name=original_name,
            ...
            await services.projects.update(project_id, body=normalize_project_body(imported_body))
        ...
        await rollback_portable_import(
            ...
            destination,
            services=services,
            delete_project_asset_files=delete_project_asset_files,
        )
        raise HTTPException(400, "无效的 LiteCut 便携工程包") from exc
    except Exception:
        await rollback_portable_import(
